...
function Invoke-F3BProcess([string]$FileName, [string[]]$Arguments, [string]$WorkingDirectory, [int]$TimeoutSeconds, [string]$InputText = '') {
    ...
    try {
        ...
        if (-not $process.WaitForExit($TimeoutSeconds * 1000)) { try { $process.Kill($true) } catch {}; $process.WaitForExit(); throw 'PROCESS_TIMEOUT' }
...
function Get-F3BGitState($Agent, [switch]$RequireClean) {
    ...
    if (-not (Get-F3BFullPath $top).Equals((Get-F3BFullPath $Agent.git_top_level), [StringComparison]::OrdinalIgnoreCase) -or $branch -cne $Agent.branch -or $head -cne $Agent.expected_head) { throw "GIT_IDENTITY_MISMATCH: $($Agent.agent_id)" }
...
function Assert-F3BImplementationReport($Report, $Agent, [string]$TaskId, [string[]]$MeasuredFiles, $PreState, $PostState) {
    ...
    if (-not (Get-F3BFullPath $Report.observed_worktree).Equals((Get-F3BFullPath $Agent.worktree_absolute_path), [StringComparison]::OrdinalIgnoreCase) -or
        $Report.observed_branch -cne $PostState.branch -or $Report.observed_head -cne $PostState.head -or
...
function New-F3BAgentStatus($Brief, $Agent) {
    [ordered]@{schema_version='1.0';task_id=$Brief.task_id;agent_id=$Agent.agent_id;planned_action=$Agent.disposition;started=$false;completed=$false;exit_code=$null;validation_stage='NOT_STARTED';success=$false;reason_code='NOT_STARTED';error_summary='';preflight=$null;postflight=$null}
}

function Write-F3BAgentStatus([string]$RunDirectory, $Status) {
    $path=Join-Path $RunDirectory "$($Status.agent_id)-status.json"
    if ([IO.File]::Exists($path)) { [IO.File]::Delete($path) }
    Write-F3BJson $path $Status
...
function Start-F3BAgent($Agent, [string]$RunDirectory, [string]$TaskId, [string]$Executable, [string[]]$Prefix, [string]$Role) {
    $preState = Get-F3BGitState $Agent -RequireClean
    $reportPath = Join-Path $RunDirectory "$($Agent.agent_id)-report.json"
    $prompt = "Phase 3B $Role. Task $TaskId. Follow the schema and Task Brief. Do not stage or commit. Return only JSON."
    ...
    $result = Invoke-F3BProcess $Executable $args $Agent.worktree_absolute_path 600 $prompt
    if ($result.ExitCode -ne 0) { throw "AGENT_FAILED: $($Agent.agent_id)" }
    $report = Read-F3BSchemaJson $reportPath $(if ($Role -ceq 'REVIEW') {'phase3b-write-review-report.schema.json'} else {'phase3b-implementation-report.schema.json'})
    if ($report.task_id -cne $TaskId) { throw 'REPORT_TASK_MISMATCH' }
    [pscustomobject]@{ Agent=$Agent; PreState=$preState; Report = $report; ReportPath = $reportPath; Process = $result }
}

function Invoke-FairiesPhase3BWriteRuntime {
    $safeRunDirectory = $null; $runId = $null; $brief = $null; $digest = $null; $stage = 'PREFLIGHT'; $bundleDigest = $null
    trap {
        if ($null -ne $safeRunDirectory -and [IO.Directory]::Exists($safeRunDirectory)) {
            try {
                $blocked = [ordered]@{ schema_version='1.0'; run_id=$runId; task_id=$brief.task_id; baseline_sha=$brief.baseline_sha; task_brief_sha256=$digest; stage='BLOCKED'; reason_code='FAIL_CLOSED'; safe_artifact_directory=$safeRunDirectory; artifact_bundle_sha256=$bundleDigest; final_result='BLOCKED' }
                $manifestPath = Join-Path $safeRunDirectory 'result-manifest.json'
                if (-not [IO.File]::Exists($manifestPath)) { Write-F3BJson $manifestPath $blocked }
            } catch { }
        }
        throw
    }
    if ([string]::IsNullOrWhiteSpace($TaskBriefPath) -or [string]::IsNullOrWhiteSpace($ApprovalPath)) { throw 'INPUT_PATH_REQUIRED' }
    $brief = Read-F3BSchemaJson $TaskBriefPath 'phase3b-write-task-brief.schema.json'
    ...
    foreach ($agent in $brief.agents | Where-Object disposition -CEQ RUN) { [void](Get-F3BGitState $agent -RequireClean) }
    if ((Get-F3BFileSha256 $TaskBriefPath) -cne $digest -or (Read-F3BSchemaJson $ApprovalPath 'phase3b-write-approval.schema.json').task_brief_sha256.ToLowerInvariant() -cne $digest) { throw 'TASK_BRIEF_OR_APPROVAL_CHANGED' }
    $stage = 'WORKER_EXECUTION'
    $attempts = [Collections.Generic.List[object]]::new()
    if ($brief.execution_strategy -ceq 'PARALLEL') {
        # Start both before validating either result; every started process is synchronously collected.
        foreach ($agent in $workers) { try { $attempts.Add((Start-F3BAgent $agent $runDirectory $brief.task_id $CodexExecutable $CodexPrefixArgument 'IMPLEMENTATION')) } catch { $attempts.Add([pscustomobject]@{ Error = $_.Exception.Message; Agent = $agent }) } }
        if (@($attempts | Where-Object { $_.PSObject.Properties.Name -contains 'Error' }).Count) { throw 'PARALLEL_WORKER_FAILED' }
    } else {
        foreach ($id in $brief.sequential_order) { $agent = $workers | Where-Object agent_id -CEQ $id; $attempts.Add((Start-F3BAgent $agent $runDirectory $brief.task_id $CodexExecutable $CodexPrefixArgument 'IMPLEMENTATION')) }
    }
    $allFiles = [ordered]@{}
    $bundles = [ordered]@{}
    foreach ($agent in $workers) {
        ...
        Assert-F3BImplementationReport $attempt.Report $agent $brief.task_id $allFiles[$agent.agent_id] $attempt.PreState $state
    ...
    foreach ($agent in $workers) { foreach ($command in @($agent.test_commands)) {
        ...
        $testEvidence.Add([ordered]@{agent_id=$agent.agent_id;command_id=$command.command_id;executable=$command.executable;argv=@($command.argv);working_directory=$logicalWd;timeout_seconds=$command.timeout_seconds;started_utc=$started.ToString('o');completed_utc=$completed.ToString('o');duration_ms=[long]($completed-$started).TotalMilliseconds;exit_code=$test.ExitCode;timed_out=$false;stdout=$stdout;stderr=$stderr})
        if ($test.ExitCode -ne 0) { throw "TEST_FAILED: $($command.command_id)" }
    } }
    ...
    if ((Get-F3BFileSha256 $TaskBriefPath) -cne $digest -or (Read-F3BSchemaJson $ApprovalPath 'phase3b-write-approval.schema.json').task_brief_sha256.ToLowerInvariant() -cne $digest) { throw 'TASK_BRIEF_OR_APPROVAL_CHANGED' }
    $reviewAttempt = Start-F3BAgent $review $runDirectory $brief.task_id $CodexExecutable $CodexPrefixArgument 'REVIEW'
    if ($reviewAttempt.Report.reviewed_artifact_sha256.ToLowerInvariant() -cne $bundleDigest -or $reviewAttempt.Report.review_verdict -cne 'APPROVE' -or $reviewAttempt.Report.final_result -cne 'READY_FOR_PHASE3C' -or @($reviewAttempt.Report.findings | Where-Object severity -in @('BLOCKER','MAJOR')).Count) { throw 'REVIEW_REJECTED' }
